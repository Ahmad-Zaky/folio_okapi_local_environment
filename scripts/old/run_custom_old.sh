#!/bin/bash

# set -e

# Okapi Url
U=http://localhost:9130

# Test Tenant
T=testlib1

# Test User
username=testing_admin
password=admin

# Store new tenant
curl -d"{\"id\":\"$T\"}" $U/_/proxy/tenants
echo -e ""

# Set Environment Variables to Okapi
curl -d'{"name":"DB_HOST","value":"localhost"}' $U/_/env && echo -e ""
curl -d'{"name":"DB_PORT","value":"5432"}' $U/_/env && echo -e ""
curl -d'{"name":"DB_USERNAME","value":"okapi"}' $U/_/env && echo -e ""
curl -d'{"name":"DB_PASSWORD","value":"okapi25"}' $U/_/env && echo -e ""
curl -d'{"name":"DB_DATABASE","value":"okapi"}' $U/_/env && echo -e ""
curl -d'{"name":"KAFKA_PORT","value":"9092"}' $U/_/env && echo -e ""
curl -d'{"name":"KAFKA_HOST","value":"localhost"}' $U/_/env && echo -e ""
curl -d"{\"name\":\"OKAPI_URL\",\"value\":\"$U\"}" $U/_/env && echo -e ""
curl -d'{"name":"ELASTICSEARCH_URL","value":"http://localhost:9200"}' $U/_/env && echo -e ""

# Clone and compile modules
compile_module() {
	# clone from github
	local m=$1
	if test ! -d $m; then	
		git clone --recurse-submodules git@github.com:folio-org/$m

	fi
	if test ! -d $m; then
		echo "$m missing. git clone failed?"
		exit 1
	fi

	# compile
	cd $m
	mvn -DskipTests -Dmaven.test.skip=true verify
	cd ..
}

# Register (store) module into Okapi
register_module() {
	local m=$2
	echo "Register module $m"
	local md=$m/target/ModuleDescriptor.json
	if test ! -f $md; then
		compile_module $m
	fi

	if test ! -f $md; then
		echo "$md missing pwd=`pwd`"
		exit 1
	fi

	if test "$1" != "x"; then
		OPT=-HX-Okapi-Token:$1
	else
		OPT=""
	fi

	curl -s $OPT -d@$md $U/_/proxy/modules -o /dev/null
	echo -e ""
}

# Deploy module into Okapi
deploy_module() {
	local m=$2
	echo "Deploy module $m"
	if test "$1" != "x"; then
		OPT=-HX-Okapi-Token:$1
	else
		OPT=""
	fi
	local dd=$m/target/DeploymentDescriptor.json
	curl -s $OPT -d@$dd $U/_/deployment/modules -o /dev/null
	echo -e ""
}

# Register and deploy modules one by one
deploy_modules() {

	# Register modules one by one
	for m in $2; do
		register_module $1 $m
	done

	# Deploy modules one by one
	for m in $2; do
		deploy_module $1 $m
	done
}

# Enable (install) modules for a tenant
install_modules() {

	# Build Body Json list of modules with action enable comes as argument
	local j="["
	local sep=""
	for m in $3; do
		j="$j $sep {\"action\":\"$2\",\"id\":\"$m\"}"
		sep=","
	done
	j="$j]"

	# Set Okapi Token if set
	if test "$1" != "x"; then
		OPT=-HX-Okapi-Token:$1
	else
		OPT=""
	fi

	# Install (enable) modules
	curl -s $OPT "-d$j" "$U/_/proxy/tenants/$T/install?purge=true"
	echo -e ""
}

# Basic Okapi curl boilerplate
okapi_curl() {
	if test "$1" != "x"; then
		local OPT="-HX-Okapi-Token:$1"
	else
		local OPT="-HX-Okapi-Tenant:$T"
	fi
	
	shift

	curl -s $OPT -HContent-Type:application/json $*
	echo -e ""
}

# New admin user with all permissions
make_adminuser() {
	local username=$2
	local password=$3
	
	# Delete admin user firstly if exists
	okapi_curl $1 -XDELETE "$U/users?query=username%3D%3D$username"
	echo -e ""

	# New admin user
	uid=`uuidgen`
	okapi_curl $1 -d"{\"username\":\"$username\",\"id\":\"$uid\",\"active\":true}" $U/users
	echo -e ""

	okapi_curl $1 -d"{\"username\":\"$username\",\"userId\":\"$uid\",\"password\":\"$password\"}" $U/authn/credentials
	echo -e ""
	
	# Set permissions for the new admin user
	puid=`uuidgen`
	okapi_curl $1 -d"{\"id\":\"$puid\",\"userId\":\"$uid\",\"permissions\":[\"okapi.all\",\"perms.all\",\"users.all\",\"login.item.post\",\"perms.users.assign.immutable\"]}" $U/perms/users
	echo -e ""
}

# Login to obtain the token from the header
login_admin() {
	
	echo -e "username: $username"
	echo -e "password: $password"
	echo -e ""

	curl -s -Dheaders -HX-Okapi-Tenant:$T -HContent-Type:application/json -d"{\"username\":\"$username\",\"password\":\"$password\"}" $U/authn/login
	echo -e ""
	
	token=`awk '/x-okapi-token/ {print $2}' <headers|tr -d '[:space:]'`
	echo -e ""

}

# Search in arguments
hasArg() {
	args=$1
	find=$2
	
	for arg in $args; do
		if [[ "$arg" =~ "$find" ]]; then
				return 1
			break
		fi
	done

	return 0
}

# get file content
getFileContent() {
	# Get the name of the JSON file.
	file_name=$1

	# Read the contents of the JSON file.
	json_data="$(cat $file_name)"
}

######################## 
# START - CORE MODULES #
########################

# Set of modules that are necessary to bootstrap admin user
CORE_MODULES="mod-users mod-login mod-permissions mod-configuration"

# Deploy and Install Core modules
deploy_modules x "$CORE_MODULES"
install_modules x enable "$CORE_MODULES"

# Install and Enable okapi module to test tenant
install_modules x enable okapi

# New admin user
make_adminuser x $username $password

# Deploy and install mod-authtoken after creating new user,
# as it will filters all incomming requests and will prevent from admin user.
deploy_modules x mod-authtoken
install_modules x enable mod-authtoken

# Login with new admin user
login_admin

###################### 
# END - CORE MODULES #
######################



######################## 
# START - TEST MODULES #
########################


# TEST_MODULES="$*"
# TEST_MODULES="mod-inventory-storage mod-password-validator mod-event-config mod-pubsub mod-circulation-storage mod-template-engine mod-email mod-sender mod-notify mod-users-bl mod-mymodule mod-assets mod-mylibrary"
TEST_MODULES="mod-password-validator mod-pubsub mod-email mod-users-bl mod-mylibrary"

# Deploy and Install Test modules
deploy_modules $token "$TEST_MODULES"
install_modules $token enable "$TEST_MODULES"

hasArg "$TEST_MODULES" "mod-users-bl"
found=$?

# Set permissions mod-users-bl module if already installed
if [[ "$found" -eq 1 ]]; then
	echo -e "with module: mod-users-bl"
	echo -e ""

	okapi_curl $token $U/perms/users/$puid/permissions -d'{"permissionName":"users-bl.all"}'
	echo -e ""

	okapi_curl $token $U/perms/users/$puid/permissions -d'{"permissionName":"users-bl.password-reset-link.generate"}'
	echo -e ""

	login_admin
	echo -e ""

	okapi_curl $token $U/bl-users/password-reset/link -d"{\"userId\":\"$uid\"}" -o reset.json

	getFileContent reset.json
	# Check if the JSON data contains the string "requires permission".
	if grep -q "requires permission" <<< "$json_data"; then
		echo "Access for user '$username' requires permission: users-bl.password-reset-link.generate"
		exit 1
	else
		token2=`jq -r '.link' < reset.json |sed -e 's@.*reset-password/@@'`

		echo -e "Second token: $token2"

		curl -s -HX-Okapi-Token:$token2 $U/bl-users/password-reset/validate -d'{}'
		echo -e ""
	fi

else
	echo -e "without module: mod-users-bl"
	echo -e ""
fi

###################### 
# END - TEST MODULES #
######################
