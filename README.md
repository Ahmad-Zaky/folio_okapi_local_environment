...
* If you run the script with modules on specific release or version, and if you downgrade the version of one or modules later on and rerun the script with the same tenant again, you may encounter some database problems, so if you encounter a problem you may check the database problem directly, I myself encountered a database problem, which was that a database routine has been added to the module schema at sometime and when I did run the script again with an old module version, the routine has not been removed and that routine did reference a relation (table) that did not exists in the schema, so after some investigation I found the issue and I had to `DROP` that routine from my module schema.
* Removing `mod-authtoken` and `mod-permissions` from enabled tenants will be applied on all tenants in the `tenants` table not just the `test` tenant used by the script because `okapi` loops on all tenants to make sure they have the same current okapi version to upgrade old tenant okapi versions at starting and if any of these two modules were enabled on any existing tenants they will lead to failure of the script.
...
- [ ] deployment right now, works by using `DeploymentDescriptor.json` file content with the `okapi` **deploy** request, we want to have the option to take the responsibility of running the module (java project) on our own, and then hit the `okapi` **discovery** request with the `url`, `instId`, and `servId` so that `okapi` will not be responsible of deploying the module by it self.
