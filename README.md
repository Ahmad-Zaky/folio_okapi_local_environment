...
* Removing `mod-authtoken` and `mod-permissions` from enabled tenants will be applied on all tenants in the `tenants` table not juse the `test` tenant used by the script because `okapi` loops on all modules at starting and if any of these two modules were enabled on any tenant they will lead the script to fail.
